...
const requestVersion = 'a'.repeat(64);
...
function seedAuthority(database: DatabaseSync): void {
  database.prepare(`INSERT INTO dux_tenant_context (
    ...
    '25566', 'Depósito prueba', ?, ?)`)
    .run(timestamp, timestamp);
  database.prepare(`UPDATE dux_catalog_control
    SET snapshot_collection_enabled = 1, updated_by = 'test', updated_at = ?
  ...
  ) VALUES ('dux_sync_assisted_test', 'manual', 'succeeded', 'test', 1, 0, 1, 0, 0, 0, ?, ?, ?, ?)`)
  ...
  ) VALUES (1, 'dux_sync_assisted_test', ?, 'PRECIOS DEL NEGOCIO', 1, ?, ?, ?, ?)`)
    .run(currentVersion, payload, timestamp, timestamp, timestamp);
  database.prepare(`UPDATE dux_catalog_control
    SET public_catalog_enabled = 1, updated_by = 'test', updated_at = ?
    WHERE company_id = '12862'`).run(timestamp);
}
...
function insertAssistedOrder(database: DatabaseSync): void {
  ...
  ) VALUES (?, ?, 'web-request-key', ?, 'preference_pending', 'ARS', 2400, 2,
    ?, ?, 'checkout_pro', ?, ?)`)
    .run(orderId, '4'.repeat(64), fingerprint, timestamp, timestamp, requestId, fingerprint);
}

function insertAssistedLine(database: DatabaseSync, quantity = 2): void {
  ...
  ) VALUES (?, 'dux-producto-prueba-0000000000000001', 'Producto Dux actual', NULL,
    'TEST-A', ?, 1200, ?, 0, NULL, ?)`)
    .run(orderId, quantity, quantity * 1200, currentVersion);
}

function insertPickup(database: DatabaseSync): void {
  database.prepare(`INSERT INTO order_fulfillment (
    ...
    NULL, 'coordinated_pickup', 2400, 0, ?, ?)`)
    .run(orderId, timestamp, timestamp);
}

function insertAssistedLink(database: DatabaseSync): void {
...
describe('0021 assisted Dux checkout', () => {
  it('preserva fulfillment histórico y admite cotización manual sólo para correo', () => {
    ...
    try {
      ...
      database.prepare(`INSERT INTO orders (
        id, public_token_hash, checkout_idempotency_key, cart_fingerprint, status,
        currency, total_minor, item_count, created_at, updated_at, channel
      ) VALUES ('legacy-order', 'legacy-token', 'legacy-key', 'legacy-fingerprint',
        'pending', 'ARS', 1500, 1, ?, ?, 'checkout_pro')`).run(timestamp, timestamp);
      database.prepare(`INSERT INTO order_fulfillment (
        order_id, delivery_method, full_name, phone, address, locality, province,
        postal_code, total_weight_grams, shipping_tier, products_total_minor,
        shipping_minor, created_at, updated_at
      ...
      database.prepare(`INSERT INTO orders (
        id, public_token_hash, checkout_idempotency_key, cart_fingerprint, status,
        currency, total_minor, item_count, created_at, updated_at, channel
      ) VALUES ('shipping-order', 'shipping-token', 'shipping-key', 'shipping-fingerprint',
        'pending', 'ARS', 2500, 1, ?, ?, 'checkout_pro')`).run(timestamp, timestamp);
  ...
  it('convierte una solicitud aceptada sin mapeo editorial y exige identidad/precio Dux actuales', () => {
    const database = new DatabaseSync(':memory:');
    try {
      database.exec(beforeAssisted);
      database.exec(assisted);
      ...
      expect(database.prepare(`SELECT product_id, sku, quantity, unit_price_minor, provider_catalog_version
        FROM order_items WHERE order_id = ?`).get(orderId)).toEqual({
  ...
  it('no permite cobrar antes de la evidencia Dux y separa el estado financiero', () => {
    ...
    try {
      ...
      ) VALUES ('123456', ?, 'approved', 'approved', NULL, 2400, 'ARS', ?, ?, ?,
        'event-test', ?, ?)`)
  ...
  it('mantiene los vínculos legacy en cuarentena y no permite conversiones no aceptadas', () => {
    ...
    try {
      ...
      database.prepare(`INSERT INTO orders (
        id, public_token_hash, checkout_idempotency_key, cart_fingerprint, status,
        currency, total_minor, item_count, created_at, updated_at, channel
      ) VALUES ('legacy-dux-order', 'legacy-dux-token', 'legacy-dux-key', 'legacy-dux-cart',
        'preference_pending', 'ARS', 1200, 1, ?, ?, 'checkout_pro')`).run(timestamp, timestamp);
      expect(() => database.prepare(`INSERT INTO dux_order_links (
        order_id, dux_reference, dux_order_id, dux_order_number, company_id, branch_id,
        deposit_id, reservation_state, request_fingerprint, created_at, updated_at,
        verification_method
      ) VALUES ('legacy-dux-order', 'legacy:dux', NULL, NULL, '12862', '1', '25566',
