...
test('navega, busca, filtra y pagina los 510 productos compilados', async ({ page }) => {
...
test('carga fichas representativas y el detalle local diferido', async ({ page }) => {
  const observation = observePage(page);

  for (const [path, title] of [
    ['/artemisa-annua-agroecologica-x-50-gr/', 'Artemisa Annua Agroecologica x 50 gr'],
    ['/guayaba/', 'Guayaba hojas x 50 gr'],
    ['/melena-de-leon-futuro-fungi-50ml/', 'Melena de león Futuro fungi 50ml'],
  ] as const) {
    await page.goto(path);
    await expect(page.getByRole('heading', { level: 1, name: title })).toBeVisible();
    await expect(page.getByText('Precio registrado')).toBeVisible();
    await expect(page.getByText('23/07/2026', { exact: true })).toBeVisible();
    await expect(page.getByRole('heading', { level: 2, name: 'Descripción' })).toBeVisible();
    await expect(page.locator('.product-detail-image')).toHaveAttribute('loading', 'lazy');
    await expect(page.getByText(/no sustituye el asesoramiento de profesionales de la salud/iu)).toBeVisible();
    await expect(page.locator('h1')).toHaveCount(1);
  }
...
test('preserva el producto sin imagen y un producto sin descripción', async ({ page }) => {
  const observation = observePage(page);
...
test('resuelve categoría, privacidad, navegación atrás y 404', async ({ page }) => {
  const observation = observePage(page);
