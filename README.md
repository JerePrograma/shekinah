...
El navegador no decide precios, disponibilidad, peso, envío, moneda ni totales. El backend vuelve a calcular el carrito desde `catalog/internal/catalog-index.json` antes de crear un pedido.
...
La configuración externa, D1, Mercado Pago, el webhook, Cloudflare Access y el número de WhatsApp deben completarse siguiendo `docs/COMMERCE_DEPLOYMENT.md`. El diseño de entrega y retención está documentado en `docs/FULFILLMENT_AND_RETENTION.md`. Tener el código en `main` o un CI verde no implica que esas integraciones estén vinculadas o activas.
