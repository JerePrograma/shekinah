...
## Continuación del cliente — 2026-09-14

El cliente confirma una sola vez los datos del carrito. Después de recuperar o registrar una solicitud activa, la misma página consulta automáticamente su estado y muestra `Pagar con Mercado Pago` sólo cuando el servidor informa reserva confirmada, total definitivo y capacidad de checkout. El pago conserva el gesto explícito del cliente, la revalidación server-side, la idempotencia y la validación de URL de Mercado Pago. Una actualización de estado no crea otra solicitud ni inicia una preferencia.
