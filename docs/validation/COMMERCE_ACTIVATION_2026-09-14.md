...
## Continuación del cliente sin consultas manuales obligatorias

Sobre `5811e71548879c37e238f753fd65f78459ee06ac`, el usuario pidió que el cliente cargue sus datos y llegue a Mercado Pago con stock real, sin sobrecargar el proceso. Se identificó un paso evitable: la pantalla sólo actualizaba la disponibilidad del pago cuando el cliente consultaba manualmente su solicitud. El cambio permite descubrir la reserva confirmada y el total desde la misma página, sin repetir los datos ni abrir WhatsApp.

`WebOrderRequestSection` consulta el estado por el GET existente, en serie, con esperas de 15, 30 y 60 segundos y un máximo de ocho lecturas por ciclo. Pausa las consultas con la pestaña oculta y se detiene ante error, resolución terminal, incidencia o checkout disponible. La recuperación manual y la salida de la página cancelan la lectura anterior para impedir respuestas fuera de orden. La preferencia sigue requiriendo el clic explícito en `Pagar con Mercado Pago` y conserva la validación de reserva, total y dominio del proveedor. No se modificaron D1, flags, cuotas, stock, sincronización, contratos de reserva ni credenciales.

Las pruebas dirigidas de React y API aprobaron 39/39. El E2E dirigido comprobó el paso de solicitud pendiente a reserva confirmada, visualización del total y un único POST de checkout después del clic, con Mercado Pago simulado. No equivale a una reserva ni a un pago productivo. El primer intento E2E había seleccionado la etiqueta `strong` en vez del párrafo con el importe; se corrigió el selector. Las comprobaciones estáticas detectaron callbacks de prueba sin espera y la incompatibilidad de `signal: undefined` con `exactOptionalPropertyTypes`; se corrigieron sin relajar ESLint ni TypeScript. La evidencia de los intentos queda en `.wrangler/commerce-d1-rollout/20260914-customer-flow`.

El readiness productivo actualizado a las 17:36:08 UTC seguía mostrando stock obsoleto, última publicación a las 14:28:29 UTC y `PAYMENT_APPLICATION_MISMATCH`, con WEB y COMMERCE cerrados. Había dos solicitudes registradas, cero pendientes y cero incidencias financieras u operaciones Dux que requirieran atención. La pestaña Dux seguía en el login. La mejora del cliente no acredita cobro inmediato automático: siguen pendientes acceso operativo a Pedidos, reserva verificable, frescura sostenida y credencial productiva válida.

La validación integral `npm run verify` aprobó 114 archivos, 700 tests y 14 omitidos, además de 29 E2E y los verificadores de lint, TypeScript, catálogo, catálogo comercial, pesos, build, assets, seguridad y automatización. `npm run build:pages` también terminó con exit code 0, con los mismos conteos unitarios y todos sus verificadores aprobados. El self-test del migrador aprobó en Windows PowerShell 5.1. Los mocks Dux aprobaron 27/27 en PowerShell 7; su intento inicial en 5.1 fue rechazado antes de ejecutar casos por el `#requires -Version 7.0` del propio archivo. Se conservaron ambos diagnósticos y no se cambió el requisito del script. Los logs de esta continuación están separados de los smokes productivos anteriores.
