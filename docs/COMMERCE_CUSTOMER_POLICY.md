...
El cliente carga los datos una vez, conoce el total confirmado, paga en Mercado Pago después de la reserva y recupera el mismo pedido al regresar. La ampliación autorizada de [compra directa](DIRECT_CHECKOUT.md) implementa precio y stock Dux en vivo, reserva única y recuperación por referencia para retiro coordinado. Su activación requiere pruebas reales y recibos de despliegue. La liberación y finalización conservan el flujo administrativo soportado; Correo sin peso estructurado mantiene cotización. Una consulta WhatsApp no acredita un pedido ni un pago. Consultar la [operación y sus requisitos](COMMERCE_OPERATIONS.md) y registrar el resultado real del despliegue y de cada smoke.
