...
it.each(['PUT','DELETE'])('una ruta de imagen %s sin autoridad Dux no lee ni modifica R2',async method=>{
  ...
  try{
    ...
    expect(response.status).toBe(503);expect(put).not.toHaveBeenCalled();expect(get).not.toHaveBeenCalled();expect(remove).not.toHaveBeenCalled();
  }finally{db.close();}
});
