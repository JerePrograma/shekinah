...
export const onRequest: PagesFunction<Env, 'id', AdminContextData> = async ({ data, env, params, request }) => {
  ...
  if (request.method === 'DELETE') return handleAdminRequest(request, env, data, 'catalog.products.delete', async (database) => { assertSameOrigin(request, env); await deleteCatalogProduct(database, id, data.adminIdentity?.actor ?? 'unknown'); return noContentResponse(); }, target);
