...
import { getRuntimeCatalogProductDetail, patchCatalogProductInventory, updateCatalogProduct } from '../../../../server/catalog-store';
...
export const onRequest: PagesFunction<Env, 'id', AdminContextData> = async ({ data, env, params, request }) => {
  ...
  if (request.method === 'GET') return handleAdminRequest(request, env, data, 'catalog.products.read', async (database) => { const product = await getRuntimeCatalogProductDetail(database, env, id); return product === null ? jsonResponse({ error: { code: 'PRODUCT_NOT_FOUND', message: 'El producto no existe.' } }, 404) : jsonResponse({ product }); }, target);
