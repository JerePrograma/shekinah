import { deleteCatalogProduct, getCatalogProductDetail, updateCatalogProduct } from '../../../../server/catalog-store';
...
export const onRequest: PagesFunction<Env, 'id', AdminContextData> = async ({ data, env, params, request }) => {
  ...
  if (request.method === 'PUT') return handleAdminRequest(request, env, data, 'catalog.products.update', async (database) => { assertSameOrigin(request, env); const body = await readJsonBody(request, 131_072); return jsonResponse({ product: await updateCatalogProduct(database, id, body, data.adminIdentity?.email ?? 'unknown') }); }, target);
  if (request.method === 'DELETE') return handleAdminRequest(request, env, data, 'catalog.products.delete', async (database) => { assertSameOrigin(request, env); await deleteCatalogProduct(database, id, data.adminIdentity?.email ?? 'unknown'); return noContentResponse(); }, target);
