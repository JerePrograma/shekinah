import { getAdminSummary, parseAdminRange } from '../../../server/admin';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({
  ...
  return handleAdminRequest(request, env, data, 'admin.summary.read', async (database) =>
    jsonResponse(await getAdminSummary(database, parseAdminRange(request))),
  );
};
