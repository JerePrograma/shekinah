import { listAudit, parseAdminRange } from '../../../server/admin';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({ data, env, request }) => {
  ...
  return handleAdminRequest(request, env, data, 'admin.audit.list', async (database) =>
    jsonResponse(await listAudit(database, parseAdminRange(request))),
  );
};
