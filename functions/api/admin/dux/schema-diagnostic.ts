...
import { readDuxStockSchemaDiagnostic } from '../../../../server/dux-schema-diagnostic';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({
  ...
  return handleAdminRequest(
    ...
    env,
    data,
    'admin.dux.schema-diagnostic',
    async () => {
      assertSameOrigin(request, env);
      return jsonResponse({ diagnostic: await readDuxStockSchemaDiagnostic(env) });
    },
  );
};
