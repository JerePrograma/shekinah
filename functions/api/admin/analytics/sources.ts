import { getAnalyticsSources, parseAdminRange } from '../../../../server/admin';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({ data, env, request }) => {
  ...
  return handleAdminRequest(request, env, data, 'admin.analytics.sources', async (database) =>
    jsonResponse(await getAnalyticsSources(database, parseAdminRange(request))),
  );
};
