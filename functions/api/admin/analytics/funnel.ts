import { getAnalyticsFunnel, parseAdminRange } from '../../../../server/admin';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({ data, env, request }) => {
  ...
  return handleAdminRequest(request, env, data, 'admin.analytics.funnel', async (database) =>
    jsonResponse(await getAnalyticsFunnel(database, parseAdminRange(request))),
  );
};
