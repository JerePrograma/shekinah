import { getAnalyticsDevices, parseAdminRange } from '../../../../server/admin';
...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({ data, env, request }) => {
  ...
  return handleAdminRequest(request, env, data, 'admin.analytics.devices', async (database) =>
    jsonResponse(await getAnalyticsDevices(database, parseAdminRange(request))),
  );
};
