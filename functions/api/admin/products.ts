...
export const onRequest: PagesFunction<Env, string, AdminContextData> = async ({ data, env, request }) => {
  ...
  if (request.method === 'POST') return handleAdminRequest(request, env, data, 'catalog.products.create', async (database) => {
    assertSameOrigin(request, env); const body = await readJsonBody(request, 131_072); const actorEmail = data.adminIdentity?.email ?? 'unknown';
    return jsonResponse({ product: await createCatalogProduct(database, body, actorEmail) }, 201);
