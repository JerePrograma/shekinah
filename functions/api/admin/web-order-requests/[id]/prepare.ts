...
import {
  ...
  prepareAssistedCheckout,
  previewAssistedCheckout,
} from '../../../../../server/assisted-checkout';
...
export const onRequest: PagesFunction<Env, 'id', AdminContextData> = async ({
  ...
  if (typeof id !== 'string') {
    return jsonResponse({ error: { code: 'WEB_REQUEST_NOT_FOUND', message: 'No se encontró la solicitud.' } }, 404);
  }
  const target = { type: 'web_request', id } as const;
  if (request.method === 'GET') {
    return handleAdminRequest(
      request,
      env,
      data,
      'admin.web_requests.assisted_checkout_preview',
      async (database) => jsonResponse(await previewAssistedCheckout(database, env, id)),
      target,
    );
  }
  return handleAdminRequest(
