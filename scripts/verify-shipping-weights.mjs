...
const coverage = { presentation: 0, name: 0, unknown: 0 };
const ambiguousProducts = [];
for (const product of products) {
  if (product === null || typeof product !== 'object' || Array.isArray(product) || typeof product.id !== 'string' || typeof product.name !== 'string') {
  ...
  if (typeof product.presentation === 'string') {
    if (presentationWeight !== null && nameWeight !== null && presentationWeight !== nameWeight) {
      ...
      ambiguousProducts.push({
        ...
        presentationWeightGrams: presentationWeight,
        nameWeightGrams: nameWeight,
      });
    } else if (presentationWeight === null) coverage.unknown += 1;
...
console.log(JSON.stringify({
  ...
  ...coverage,
  ambiguous: ambiguousProducts.length,
  conflicts: 0,
  ambiguousProducts,
}));
